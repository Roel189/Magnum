...
public class ExtentReportUtils {
    ...
    public static void addTestResult(CustomTestWatcher watcher) {
        Throwable throwable = watcher.getThrowable();
        if (throwable != null) {
            TEST.log(Status.FAIL, "Test Result: " + Status.FAIL + "<pre>" + "Message: " + throwable.toString() + "</pre>" + "<pre>" + "Stack Trace: " + Utils.getAllStackTrace(watcher) + "</pre>", getScreenShotMedia(testNameMemory));
        } else {
            TEST.log(Status.PASS, "Test Result: " + Status.PASS);
        }
    }
    /*
    public static void addTestResult(TestWatcher result){
        switch (result.getStatus())
        {
            case TestWatcher.FAILURE:
